<template>
  <div class="tw-m-2">
    ...
    <div v-else>
      <app-button-create :to="{ name: 'user-group-create' }" />
      <app-table
      ...
      >
        <template #actions="row">
          ...
          <q-btn
            ...
            icon="create"
            label="ویرایش"
            :to="{ name: 'user-group-update-id', params: { id: row.id } }"
          />
        </template>
...
import {
  useDeleteUserGroup,
  useMultipleDeleteUserGroup,
  useUserGroupList,
} from 'src/composables/userGroup/useUserGroup';

const { columns, request, data, loading } = useUserGroupList();
const { request: deleteRequest, loading: deleting } = useDeleteUserGroup();
