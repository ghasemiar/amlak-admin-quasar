<template>
  <div class="tw-m-2">
    ...
    <div v-else>
      <app-button-create :to="{ name: 'user-create' }" />
      <app-table
      ...
      >
        <template #actions="row">
          ...
          <q-btn
            ...
            icon="create"
            label="ویرایش"
            :to="{ name: 'user-update-id', params: { id: row.id } }"
          />
        </template>
...
} from 'src/composables/user/useUser';
import axios from 'axios';

const { columns, request, data, loading } = getAllUser();
const { request: deleteRequest, loading: deleting } = useDeleteUser();
