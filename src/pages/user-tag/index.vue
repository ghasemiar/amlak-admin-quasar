<template>
  <div class="tw-m-2">
    ...
    <div v-else>
      <app-button-create :to="{ name: 'user-tag-create' }" />
      <app-table
      ...
      >
        <template #actions="row">
          ...
          <q-btn
            ...
            icon="create"
            label="ویرایش"
            :to="{ name: 'user-tag-update-id', params: { id: row.id } }"
          />
        </template>
...
import {
  useDeleteUserTag,
  useMultipleDeleteUserTag,
  useUserTagList,
} from 'src/composables/userTag/useUserTag';

const { columns, request, data, loading } = useUserTagList();
const { request: deleteRequest, loading: deleting } = useDeleteUserTag();
