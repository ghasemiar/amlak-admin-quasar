    ...
    <div class="tw-grid lg:tw-grid-cols-4 md:tw-grid-cols-2 tw-grid-cols-1 tw-gap-4">
      <app-dialog :label="$t('select related post')" class="tw-w-full" :full_size="true">
        ...
        <div class="tw-grid tw-grid-cols-4 tw-gap-5">
          <div v-for="post in getPostsData" :key="post.id">
...
import { usePost, usePostList, useUpdatePost } from 'src/composables/blog/usePost';
...
const { loading:getPostsLoading, request:getPosts, data:getPostsData } = usePostList();
const { loading:getPostLoading, request:getPost, data:getPostData } = usePost();
...
watch(getPostData, (newValue) => {
  if (newValue && Object.keys(newValue).length) {
    ...
    setFieldValue('related_post', newValue.related_post?.map((post)=>post.id));
  }
});
</script>
