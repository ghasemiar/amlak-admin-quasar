...
const { data: fetchedData, loading: fetching, request: fetch } = useLocation();
onMounted(async () => {
  fetch();
});
watch(fetchedData, (newValue) => {
  if (newValue) {
    ...
    setFieldValue('explanation', newValue.explanation);
  }
});
const { request, loading } = useUpdateLocation();
