...
// Fetch the existing data for the advertise to update
const { data: fetchedData, loading: fetching, request: fetch } = getAdvertise();
onMounted(async () => {
  fetch();
});
watch(fetchedData, (newValue) => {
  if (newValue) {
    ...
    if (newValue.type == AdvertiseType.HOUSE) {
      setFieldValue('building_age', newValue?.advertising_attr?.building_age);
      setFieldValue(
        'car_park_count',
        newValue?.advertising_attr?.car_park_count
      );
      setFieldValue('room_count', newValue?.advertising_attr?.room_count);
      setFieldValue('cooling', newValue?.advertising_attr?.cooling);
      setFieldValue('heating', newValue?.advertising_attr?.heating);
      setFieldValue('house_type', newValue?.advertising_attr?.house_type);
    }
  }
});
// Handle form with vee-validate
const { handleSubmit, setFieldValue } = useForm<AdvertiseFormDto>({
...
const { request, loading: updateLoading } = useUpdateAdvertise();
// Update function using axios
const onSubmit = handleSubmit((values) => {
  ...
  formData.append('house_type', values.house_type);
  formData.append('room_count', values.room_count);
  formData.append('building_age', values.building_age);
  formData.append('heating', values.heating);
  formData.append('cooling', values.cooling);
  formData.append('car_park_count', values.car_park_count);
